...
def data_import(myfile):
    ...
    file_loc = "./addDbNotes/db files Sep 2015/nzdc_export_" ### CHECK ADDRESS !!!
    file_type = ".txt"
    load_db = False
    action = myfile.split(":")
    try:
...
def add_citation_to_headWord(citation, headword_obj):
    ...
    if int(m) < 1:
      ...
      m = '1'
      circa = 1
    if int(y) < 1: # At least one case of year == 0, but contain day/month values
                   # year == 0 is out of range for datetime object
      y = '1'
      circa = 1
